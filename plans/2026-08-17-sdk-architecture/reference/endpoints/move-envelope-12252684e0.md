...
info:
  ...
  version: 1.0.0
paths:
  /v1/ecm/envelopes/{envelopeId}/to/{vaultId}:
    post:
      summary: Move envelope
      deprecated: false
      description: >-
        Moves an existing envelope using its unique identifier to a vault for
        secure storage. All envelopes are stored within a vault, and this
        operation ensures the envelope is properly relocated within the system.
        Be aware that the envelope's contents and associated signers are not
        affected by this action, and the envelope's unique ID remains unchanged.
        Ensure the provided envelope ID exists and is valid before performing
        this action.
      tags:
        - Envelope
        - Envelope
      ...
      responses:
        '204':
          description: Envelope moved successfully
          headers: {}
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/755605/apis/api-12252684-run
components:
  schemas:
